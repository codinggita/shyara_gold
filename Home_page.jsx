...
const JewelryStore = () => {
  ...
  useEffect(() => {
    axios.get("https://shyara-gold.onrender.com/best_selling_items") // Replace with your API endpoint
      .then(response => {
        setBestSellingItems(response.data);
      })
      .catch(error => console.error("Error fetching best selling items:", error));
  }, []);
  ...
  return (
    <div className="store-container">
      ...
      <main className="main-content">
        ...
        {/* Best Selling Items - Dynamic Cloudinary Images */}
            ...
            {bestSellingItems.length > 0 ? (
              bestSellingItems.map((item, index) => (
                <div key={index} className="item-card">
                  <img src={item.imageUrl} alt={`Best selling item ${index + 1}`} />
                </div>
              ))
            ) : (
